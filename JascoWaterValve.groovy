...
@Field static int homeSecurity = 0x07
//Notification Events
...
@Field static int tamperRemove = 0x03
//meter type
@Field static int waterMeter = 0x03
          ...
          //Map evt = [name: "watermeter", unit: "LPH"]
            if (cmd.meterType == 3) {
                  switch (cmd.scale) {
                        case 2:
                              evt.name = "rate"
                              evt.value = cmd.scaledMeterValue*3600/cmd.deltaTime*3.78541
                              evt.unit = "LPH"
...
//	return secureCmd(zwave.meterV5.meterGet(meterType: meterType, scale:0))
//}
...
void executeRefreshCmds() {
	...
	cmds << notificationGetCmd(homeSecurity)
...
void sendhomeSecurityEvent(event) {
	sendEventIfNew("tampercovering", (event == 0x03 ? "remove" : "covering"))
}

void sendWaterMeterEvent(event) {
	sendEventIfNew("Cubic meters", "water")
}

void sendBatteryEvent(event) {
	sendEventIfNew("Battery level", "%")
}
/*******************************************************************
 ***** Common Functions
********************************************************************/
